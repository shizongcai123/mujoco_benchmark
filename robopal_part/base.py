...
from .xml_splice import RobotGenerator
from .grippers import BaseEnd, REGISTERED_ENDS
import robopal_part
    ...
    def create_end_effector(self, gripper):
        ...
        if gripper_names is None:
            ...
            for agent, gripper in zip(self.agents, gripper_names):
                try:
                    self.end[agent] = REGISTERED_ENDS[gripper](self.robot_data, self.robot_model, agent)
                except KeyError:
                    logging.error(f"End {gripper} is not registered. Available robots are {REGISTERED_ENDS.keys()}.")
                    raise KeyError
