...
ROBOPAL_PATH = os.path.dirname(inspect.getfile(robopal_part))
print(f"ROBOPAL_PATH {ROBOPAL_PATH}")
        ...
        if isinstance(mount, list):
            for ch_id, ch_name in enumerate(mount):
                ...
                self.add_all_component_from_xml(
                    ...
                    is_rename_tag = not (len(mount) == 1 and len(robot) > 1)
                )

        if isinstance(robot, Iterable):
            for mani_id, mani_name in enumerate(robot):
